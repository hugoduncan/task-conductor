...
(defmacro with-clean-graph
  "Resets the pathom graph state before and after body execution."
  [& body]
  `(do
     (core/reset-graph!)
     (try
...
(deftest register!-test
  ;; Verifies that register! accepts resolvers, mutations, and collections.
  (with-clean-graph
    (testing "register!"
...
(deftest env-test
  ;; Verifies env creation, caching, and cache invalidation on registration.
  (with-clean-graph
    (testing "env"
...
(deftest query-test
  ;; Verifies EQL query execution for resolvers, entity inputs, and mutations.
  (with-clean-graph
    (testing "query"
